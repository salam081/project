...
def reset_password_view(request, identifier):
    # ✅ Only allow POST requests — prevents accidental resets via GET
    if request.method != "POST":
    ...
    # ✅ Prevent resetting another superuser's password unless you are also a superuser
    if user.is_superuser and not request.user.is_superuser:
        messages.error(request, "You cannot reset a superuser's password.")
        return redirect("all_members")

    # ✅ Generate a secure random password instead of hardcoded "pass"
    ...
    messages.success(
        ...
        f"Please share this securely with the user."
    )
    return redirect("all_members")
